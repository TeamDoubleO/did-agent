...
public class Ed25519KeyGenerator {
    ...
    public static KeyMaterial generate() throws Exception {
        ...
        return new KeyMaterial(rawPub, rawPriv, publicKeyBase58, privateKeyBase58,
                (EdECPublicKey) kp.getPublic(), (EdECPrivateKey) kp.getPrivate());
    }

    /** 결과 보관용 record (Java 16+ 기능) */
    public record KeyMaterial(
            byte[] rawPub, byte[] rawPriv,
            String pub58, String priv58,
            EdECPublicKey jdkPub, EdECPrivateKey jdkPriv
    ) {}
}
